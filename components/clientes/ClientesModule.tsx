      ...
      <CardContent className="space-y-4">
        ...
        <div className="rounded-2xl border overflow-auto">
          <table className="w-full text-sm">
            ...
            ] as Array<[SortField, string]>).map(([field, label]) => <th key={field} className="text-left px-5 py-4 uppercase tracking-wider text-slate-500 font-semibold"><button onClick={() => setSort({ field, dir: sort.field === field && sort.dir === "asc" ? "desc" : "asc" })}>{label}</button></th>)}<th className="px-5 py-4 uppercase tracking-wider text-slate-500 font-semibold">Ações</th></tr></thead>
            <tbody>{paged.map((c) => <tr key={c.id} className="border-t"><td className="px-5 py-4 font-semibold text-[#0B2A5B]">{c.nome}</td><td className="px-5 py-4">{c.cnpj || "-"}</td><td className="px-5 py-4">{c.responsavel || "-"}</td><td className="px-5 py-4">{c.regime}</td><td className="px-5 py-4">{c.segmento}</td><td className="px-5 py-4"><span className={`px-3 py-1 rounded-full text-sm font-semibold ${c.status === "Ativo" ? "bg-emerald-100 text-emerald-700" : c.status === "Especial" ? "bg-blue-100 text-blue-700" : "bg-slate-200 text-slate-700"}`}>{c.status}</span></td><td className="px-5 py-4 font-semibold">{brl(c.honorario)}</td><td className="px-5 py-4"><div className="flex gap-2"><Button variant="ghost" onClick={() => setViewing(c)}>Ver</Button><Button variant="ghost" onClick={() => openEdit(c)}>Editar</Button><Button variant="ghost" onClick={() => deleteClient(c.id)}>Excluir</Button></div></td></tr>)}</tbody>
          </table>
        </div>
