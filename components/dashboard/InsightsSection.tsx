...
export function InsightsSection({ insights = EMPTY_DASHBOARD_INSIGHTS }: { insights?: DashboardInsightsData }) {
  const safeInsights = insights ?? EMPTY_DASHBOARD_INSIGHTS;
  const topProfitable = safeInsights.topProfitable ?? [];
  const topAlerts = safeInsights.topAlerts ?? [];
  const quadrantChart = safeInsights.quadrantChart ?? [];
  const segmentChart = safeInsights.segmentChart ?? [];
  ...
  return (
    <section className="grid grid-cols-1 xl:grid-cols-2 gap-4">
      ...
      <Card className="border-0 shadow-sm">
        ...
        <CardContent className="p-0">
          {topAlerts.length === 0 ? <div className="h-[204px] flex items-center justify-center text-contflix-muted font-medium">Aguardando dados</div> : (topAlerts ?? []).map((item) => (
            <div key={item.id} className="flex items-center justify-between px-5 py-4 border-b last:border-b-0">
              <div className="flex items-center gap-3">
      ...
      <Card className="border-0 shadow-sm">
        ...
        <CardContent className="h-[320px]">
          {quadrantChart.length === 0 ? <div className="h-full flex items-center justify-center text-contflix-muted font-medium">Aguardando dados</div> : <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={quadrantChart ?? []} dataKey="value" nameKey="name" cx="42%" cy="50%" innerRadius={60} outerRadius={100}>{(quadrantChart ?? []).map((entry) => <Cell key={entry.name} fill={entry.color} />)}</Pie><Tooltip /><Legend layout="vertical" align="right" verticalAlign="middle" /></PieChart></ResponsiveContainer>}
        </CardContent>
      </Card>
      ...
      <Card className="border-0 shadow-sm">
        ...
        <CardContent className="h-[320px]">
          {segmentChart.length === 0 ? <div className="h-full flex items-center justify-center text-contflix-muted font-medium">Aguardando dados</div> : <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={segmentChart ?? []} dataKey="value" nameKey="name" cx="42%" cy="50%" innerRadius={60} outerRadius={100}>{(segmentChart ?? []).map((entry) => <Cell key={entry.name} fill={entry.color} />)}</Pie><Tooltip /><Legend layout="vertical" align="right" verticalAlign="middle" /></PieChart></ResponsiveContainer>}
        </CardContent>
      </Card>
    </section>
  );
